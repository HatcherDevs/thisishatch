...
@section('content')
    <main id="main-content" class="main-content event-section about_page">
        <section class="row mb-5">
            @if ($repository && $repository->image)
                <div class="col-12 header_img">
                    <img class="border-top border-3 border-black"
                        src="{{ RvMedia::getImageUrl($repository->image, null, false, RvMedia::getDefaultImage()) }}"
                        alt="{{ $repository->title }}" style="width: 100%; height: 100%; object-fit: cover; display: block" />
                </div>
            @endif

            <div class="col-12">
                <hr class="divider" />
                <h2 class="my-1" style="font-size: 26px">{{ $repository->title }}</h2>
                <hr class="divider" />
                <div class="row">
