@php
    ...
    if ($coverImage) {
        if (filter_var($coverImage, FILTER_VALIDATE_URL)) {
            ...
            $posterImage = Theme::asset()->url(
                'imgs/' . ltrim($coverImage, '/')
            );
        } else {
            $posterImage = RvMedia::getImageUrl(
                $coverImage,
                null,
                false,
                $defaultPoster
    ...
    $autoPlay = in_array(
        strtolower((string) ($shortcode->autoplay ?? '0')),
        ['1', 'true', 'yes'],
        true
    );

    $mute = in_array(
        strtolower((string) ($shortcode->mute ?? '1')),
        ['1', 'true', 'yes'],
        true
    );

    $loop = in_array(
        strtolower((string) ($shortcode->loop ?? '0')),
        ['1', 'true', 'yes'],
        true
    );

    $headline = $shortcode->headline
