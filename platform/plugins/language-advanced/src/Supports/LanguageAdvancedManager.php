            ...
            foreach ($columns as $column) {
                MacroableModels::addMacro(
                    ...
                    function () use ($column) {
                        ...
                        $isCurrentDefaultLocale = $currentLocale == Language::getDefaultLocaleCode();

                        if (
                            ! $model->lang_code &&
                            ! $isCurrentDefaultLocale &&
                            $translation = $model->translations->where('lang_code', $currentLocale)->value($column)
                        ) {
                            return $translation;
                        }
