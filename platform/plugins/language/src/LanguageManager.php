...
class LanguageManager
{
    ...
    public function setLocale(?string $locale = null): ?string
    {
        ...
        if (empty($locale) || ! is_string($locale)) {
            ...
            // Check if we're in admin area, use admin_language session
            $isAdmin = $this->request->is(config('core.base.general.admin_dir') . '*');
            $locale = session($isAdmin ? 'admin_language' : 'language');
        ...
        if (array_key_exists($locale, $supportedLocales)) {
            ...
            // $locale = null;
            $isAdmin = $this->request->is(config('core.base.general.admin_dir') . '*');
            $localeFromSession = session($isAdmin ? 'admin_language' : 'language');
