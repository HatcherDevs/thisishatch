...
class Theme implements ThemeContract
{
    ...
    {
        // dd($this->regions['down_row'] );
        return $this->regions['down_row'] ?? null;
    }

    /**
     * Get page data from arguments.
     */
    public function page(): mixed
    {
        return $this->arguments['page'] ?? null;
    }
    ...
    public function of(string $view, array $args = []): self
    {
        ...
        if (isset($args['page']) && (! empty($args['page']->down_row) || $args['page']->down_row === null)) {
            ...
            if ($downRow) {
                $this->regions['down_row'] = apply_filters(
                    'page_front_down_row',
                    Html::tag('div', BaseHelper::clean($downRow), ['class' => 'ck-content'])->toHtml(),
                    $args['page']
                );
            }
        }
