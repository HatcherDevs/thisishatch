    ...
    public function generateMenu(array $args = []): ?string
    {
        ...
        if ($cacheEnabled && $this->cache->has($cacheKey) && ($cachedData = $this->cache->get($cacheKey))) {
            ...
            if ($menuNodes instanceof Collection) {
                try {
                    // Load reference and all nested child relationships recursively
                    $menuNodes->loadMissing([
                        'reference',
                        'child.reference',
                        'child.child.reference',
                        'child.child.child.reference',
                        'child.child.child.child.reference',
                    ]);
                } catch (Throwable) {
                }
            }
            ...
            // Pass through any additional args to the view (like parentLevel)
            if (Arr::has($args, 'parentLevel')) {
                $data['parentLevel'] = Arr::get($args, 'parentLevel');
            }
