...
class SlugService
{
    ...
    {
        // Handle nested slugs (with forward slashes) differently
        if (str_contains($name, '/')) {
            // Split by slash and slugify each part individually
            $parts = explode('/', $name);
            $slugParts = [];
            ...
            foreach ($parts as $part) {
                $part = trim($part);
                if ($part !== '') {
                    $slugParts[] = Str::slug($part, '-', !SlugHelper::turnOffAutomaticUrlTranslationIntoLatin() ? 'en' : false);
                }
            }

            $slug = implode('/', $slugParts);
        } else {
            // Regular slug without slashes
